...
class TINRasterSourceSpec extends FunSpec with RasterMatchers {
  ...
  describe("TINRasterSourceSpec") {
    it("should read from the EPT catalog") {
      ...
      val rs = TINRasterSource(catalog, threads = Some(1))
    ...
    it("should read from the EPT catalog at a less resolute level of detail") {
      ...
      val rs = TINRasterSource(catalog, threads = Some(1), overviewStrategy = Auto(6))
    ...
    it("should resample RasterSource") {
      ...
      val rs = TINRasterSource(catalog, threads = Some(1)).resample(100, 100)
    ...
    it("should reproject RasterSource") {
      ...
      val rs = TINRasterSource(catalog, threads = Some(1)).reproject(LatLng)
    ...
    it("rasterizer bug") {
      ...
      val rs = TINRasterSource(catalog, threads = Some(1)).resampleToRegion(ge)
